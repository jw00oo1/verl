...
@ToolParser.register("glm-like")
class GlmStyleToolParser(ToolParser):
    """Tool parser for GLM-like tool-call formats wrapped by ``<tool_call>`` tags."""
    ...
    def __init__(self, tokenizer) -> None:
        ...
        self.tool_call_end_token: str = "</tool_call>"
        self.tool_call_regex = regex.compile(r"<tool_call>(.*?)</tool_call>", regex.DOTALL)
        self.name_regex = regex.compile(r"^\s*(\S+)")
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for block in matches:
            try:
                name_match = self.name_regex.search(block)
                ...
                args: dict[str, str] = {}
                # Duplicate keys are overwritten by later values ("last value wins").
                for key, value in self.arg_pair_regex.findall(block):
                    args[key] = value

                function_calls.append(FunctionCall(name=name, arguments=json.dumps(args, ensure_ascii=False)))
            except Exception as e:
                logger.warning(f"Skipping malformed GLM-like tool call block: {e}")
